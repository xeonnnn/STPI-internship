...
class ConferenceRegistration(models.Model):
    """Stores ICIMMI registration details submitted before payment."""
    ...
    DESIGNATION_CHOICES = [
        ('professor', 'Professor/Assoc Prof/Asst Professor'),
        ('industry', 'Industry Expert'),
        ('researcher', 'Researcher/PhD Scholar'),
        ('pg', 'PG Student'),
        ('ug', 'UG Student'),
    ]
    PRESENTATION_MODE_CHOICES = [
        ('online', 'Online'),
        ('offline', 'PIET, Jaipur - Offline (Mandatory for Local Participants)'),
    ]
    PUBLICATION_CHOICES = [
        ('acm', 'ACM-ICPS (Scopus Conference Proceedings, Open Access)'),
        ('taru', 'Taru-JDMSC (Scopus Q1, Journal, Additional APC Applicable)'),
    ]
    INTERNATIONAL_AUTHOR_CHOICES = [
        ('yes', 'Yes'),
        ('no', 'No'),
    ]
    MEMBERSHIP_CHOICES = [
    ...
    paper = models.OneToOneField(Paper, on_delete=models.CASCADE, related_name='registration')
    submitted_by = models.ForeignKey(User, on_delete=models.CASCADE, related_name='conference_registrations')
    created_at = models.DateTimeField(auto_now_add=True)
    ...
    # Authors
    num_authors = models.CharField(max_length=10, choices=NUM_AUTHORS_CHOICES)
    num_authors_other = models.CharField(max_length=10, blank=True)
    ...
    designation_first_author = models.CharField(max_length=20, choices=DESIGNATION_CHOICES, verbose_name='Designation of 1st Author')
    affiliation_corresponding = models.CharField(max_length=255, verbose_name='Affiliation of Corresponding Author')

    # International
    has_international_author = models.CharField(max_length=3, choices=INTERNATIONAL_AUTHOR_CHOICES, verbose_name='Any International Author?')
    affiliation_international = models.CharField(max_length=255, blank=True, verbose_name='Affiliation of International Author (if any)')

    # Presentation & Publication
    presentation_mode = models.CharField(max_length=20, choices=PRESENTATION_MODE_CHOICES)
    publication_choice = models.CharField(max_length=10, choices=PUBLICATION_CHOICES)

    # Payment
    amount_paid = models.CharField(max_length=50, verbose_name='Amount Paid for Registration (INR/USD)')

    # Membership
    has_membership = models.CharField(max_length=3, choices=MEMBERSHIP_CHOICES, verbose_name='IEEE/ACM/FIP/CSI/IETE Membership?')
    membership_proof = models.FileField(upload_to='registration/membership/', blank=True, null=True, verbose_name='Membership Proof')

    # File uploads
    revised_paper = models.FileField(upload_to='registration/revised_papers/', blank=True, null=True, verbose_name='Word/Latex Revised Paper')
    pdf_copy = models.FileField(upload_to='registration/pdf_copies/', blank=True, null=True, verbose_name='PDF Copy')
    plagiarism_report = models.FileField(upload_to='registration/plagiarism/', blank=True, null=True, verbose_name='Plagiarism Report')

    def __str__(self):
        return f"Registration: {self.paper.paper_id} by {self.submitted_by.username}"
