...
@login_required
def conference_register(request, paper_id):
    """Display ICIMMI registration form; on POST save data and redirect to payment."""
    from .models import ConferenceRegistration
    paper = get_object_or_404(Paper, id=paper_id, author=request.user)

    # If already registered, go straight to payment
    if hasattr(paper, 'registration'):
        return redirect('conference:register_payment', paper_id=paper.id)

    if paper.status != 'accepted':
        messages.error(request, 'Registration is only available for accepted papers.')
        return redirect('conference:author_papers', conference_id=paper.conference.id)
    ...
    if request.method == 'POST':
        ...
        try:
            reg = ConferenceRegistration(
                ...
                num_authors=request.POST.get('num_authors', ''),
                num_authors_other=request.POST.get('num_authors_other', ''),
                name_first_author=request.POST.get('name_first_author', ''),
                affiliation_first_author=request.POST.get('affiliation_first_author', ''),
                designation_first_author=request.POST.get('designation_first_author', ''),
                affiliation_corresponding=request.POST.get('affiliation_corresponding', ''),
                has_international_author=request.POST.get('has_international_author', 'no'),
                affiliation_international=request.POST.get('affiliation_international', ''),
                presentation_mode=request.POST.get('presentation_mode', ''),
                publication_choice=request.POST.get('publication_choice', ''),
                amount_paid=request.POST.get('amount_paid', ''),
                has_membership=request.POST.get('has_membership', 'no'),
            )
            if 'membership_proof' in request.FILES:
                reg.membership_proof = request.FILES['membership_proof']
            if 'revised_paper' in request.FILES:
                reg.revised_paper = request.FILES['revised_paper']
            if 'pdf_copy' in request.FILES:
                reg.pdf_copy = request.FILES['pdf_copy']
            if 'plagiarism_report' in request.FILES:
                reg.plagiarism_report = request.FILES['plagiarism_report']
            reg.save()
            messages.success(request, 'Registration details saved! Please complete the payment.')
            return redirect('conference:register_payment', paper_id=paper.id)
...
@login_required
def register_payment(request, paper_id):
    """Show payment page after registration form is submitted."""
    from .models import ConferenceRegistration
    paper = get_object_or_404(Paper, id=paper_id, author=request.user)
    registration = getattr(paper, 'registration', None)
    if not registration:
        return redirect('conference:conference_register', paper_id=paper.id)
    return render(request, 'conference/register_payment.html', {
        'paper': paper,
